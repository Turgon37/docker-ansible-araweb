...
application_version=$(docker inspect -f '{{ index .Config.Labels "application.araweb.version" }}' "${image_building_name}")
publish=false
if [[ "${VCS_BRANCH}" == "${PRODUCTION_BRANCH}" ]]; then
  image_tags=("${application_version}-latest" "${application_version}-${image_version}")
